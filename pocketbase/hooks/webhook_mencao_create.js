onRecordAfterCreateSuccess(async (e) => {
  ...
  const monitoramentoId = e.record.getString('monitoramento_id')

  try {
    const mon = $app.findRecordById('monitoring', monitoramentoId)
    if (mon.getString('tipo') !== 'mencao') {
      return e.next()
    }

    const gatilho = 'mencao'
